 ...
 */
export async function generateInvoicePdf(orderId: number): Promise<{ buffer: Buffer; filename: string }> {
  ...
  // Items table
  const tableHead = [['#', 'Product Description', 'HSN/SAC', 'Qty', 'Unit', 'Rate', 'Taxable', 'GST %', 'GST Amt', 'Total']];
  ...
  autoTable(doc, {
    ...
    body: tableBody,
    theme: 'grid',
    styles: { fontSize: 7.5, cellPadding: 2, lineColor: [203, 213, 225], lineWidth: 0.3, textColor: [30, 41, 59] },
    headStyles: { fillColor: [30, 41, 59], textColor: [255, 255, 255], fontStyle: 'bold', fontSize: 7 },
    alternateRowStyles: { fillColor: [248, 250, 252] },
    columnStyles: {
      0: { cellWidth: 8, halign: 'center' }, 3: { cellWidth: 12, halign: 'center' },
      4: { cellWidth: 12, halign: 'center' }, 5: { cellWidth: 20, halign: 'right' },
      6: { cellWidth: 22, halign: 'right' }, 7: { cellWidth: 14, halign: 'center' },
      8: { cellWidth: 20, halign: 'right' }, 9: { cellWidth: 22, halign: 'right' },
    },
  ...
  const boxW = contentW;
  const boxH = 38;
  if (y + boxH + 30 > doc.internal.pageSize.getHeight()) { doc.addPage(); y = margin; }
